...
class Rating {
  ...
  private async _buildModalRating(): Promise<void> {
    ...
    this._users = ratings.entries;
    this._userScore = { place: ratings.entries.find((el => el.player.uniqueID === User.getID())).rank };
    ...
    this._users.forEach((user, i) => {
      const username = user?.player.publicName ? user?.player.publicName : 'Неизвестный ловитель яиц'
      const name = this._scene.add.text(
        this._modal.getBounds().left + 20,
        this._modal.getBounds().top + 100 + 60 * (i + 1),
        (i + 1 + ". " + username).toUpperCase(),
        {
          align: 'left',
          color: 'black',
